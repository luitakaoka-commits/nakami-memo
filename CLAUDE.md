...
# recipe 16件 / recipe-stock 12件 / app-switcher 26件 / install 10件 / rules 7件 / migrate 7件 / recipes 26件 も同じコマンドで走る
...
- **Phase 0〜4 はすべて完了し、ユーザーが本番で動作確認済み。** 買う（レシート取込）→ 在庫に入れる →
...
GitHub Desktop の「Push origin」。`main` への push は Vercel の本番デプロイになります。
...
**2026-09-15 完了（本番で動作確認済み）。** ユーザーが実機で、レシピ提案→つくりおきノートに保存→「作った」→なかみメモの在庫が減る、まで確認。

| 場所 | 中身 |
|---|---|
| `src/lib/recipes/suggest-core.ts` | ネットワークを使わない部分すべて（在庫の絞り込み・AIへの指示・返答の突き合わせ・単位の換算・保存形・減らす量の初期値・提案の一時保存）。テスト26件 |
...
- **つくりおきノート側の「作った」でも在庫を減らせる**（2026-09-15 ユーザー要望）。保存時に `sourceItems`
  ...
  つくりおきノートの `normalizeRecipe` は読み込みのたびにレシピを作り直すので、**`sourceItems` をそこで拾い直すこと**
  （落とすと確認画面が出ない。2026-09-15 の不具合。`tests/cook-stock.test.js` が回帰を止める）
...
| B | なかみメモで「使い切った／捨てた」→ 数量0のまま残す ＋ お金管理の明細へ書き戻す | 済（2026-09-16 実機確認済み） |
| C | つくりおきノートの買い物リストで「これ、まだ家にあります」 | 済（2026-09-16 実機確認済み） |
...
| 場所 | 中身 |
|---|---|
| `public/shared/pantry-check.js` | 買い物リストと在庫の突き合わせ（純粋）。テストは `public/recipe/tests/pantry-check.test.js` 20件 |
| `public/recipe/app.js` の `ensurePantry` / `renderShopping` | 買い物リストを開いたときに在庫を1回読み、未購入の行に印を出す |

- **出すだけ。消しも減らしもしない**（買う理由は本人にしか分からない。特売でまとめ買いもある）
- 買ったもの（done）には出さない。言われても遅い
- 品名の突き合わせは**お金管理の `normalizeItemName` を借りる**。つくりおきノートは ESM なので
  `await import("../money/finance-engine.js")` で読み、`window.FinanceEngine` から使う（IIFE なので import しても副作用で入る）。
  **同じ表記ゆれの吸収を2か所に書かないため。** 80KB あるので買い物リストを開いたときだけ読む。
  テストも `require` で本物を読んで動かす（自前の簡易版で代用すると、本番だけ引けない失敗を見逃す）
- 別名辞書（`itemAliases`）はお金管理の workspace の中なので、つくりおきノートからは引かない。
  機械的な正規化だけ。「牛乳」と「低脂肪乳」は別のまま（意味の判断はしない、の方針どおり）
