    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-3xl bg-white shadow-soft">
        <div className="relative h-52 bg-slate-100">
          {location.imageUrl ? <Image src={location.imageUrl} alt={location.name} fill sizes="100vw" className="object-cover" /> : <div className="flex h-full items-center justify-center text-slate-400">No Image</div>}
        </div>
        <div className="p-5">
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
            <div>
              ...
              <h1 className="mt-3 text-2xl font-black text-slate-900">{location.name}</h1>
              {location.memo && <p className="mt-2 text-sm leading-6 text-slate-600">{location.memo}</p>}
            </div>
            <div className="flex shrink-0 flex-wrap gap-2">
              <Link href={`/app/items/new?locationId=${location.id}`} className="rounded-xl bg-brand-600 px-4 py-2 text-sm font-semibold text-white">アイテム追加</Link>
              <Link href={`/app/locations/${location.id}/edit`} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700">編集</Link>
              <Link href={`/app/locations/${location.id}/qr`} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700">QR</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-3">
        <div className="flex items-end justify-between">
          <div>
            <h2 className="text-xl font-bold text-slate-900">中身一覧</h2>
            <p className="text-sm text-slate-500">名前順で表示しています。</p>
          </div>
          <span className="text-sm font-semibold text-slate-500">{items.length}件</span>
        </div>

        {items.length === 0 ? (
          <EmptyState title="まだ中身がありません" description="何が入っているか、まず1つ登録しましょう。" actionLabel="アイテムを追加" href={`/app/items/new?locationId=${location.id}`} />
        ) : (
